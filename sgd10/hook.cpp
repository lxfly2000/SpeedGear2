...
BOOL GetPresentVAddr(PFIDXGISwapChain_Present* pPresent, PFIDXGISwapChain_ResizeBuffers* pResizeBuffers)
{
	...
	HRESULT hr = D3D10CreateDeviceAndSwapChain(NULL, D3D10_DRIVER_TYPE_HARDWARE, NULL, device_flag,D3D10_SDK_VERSION,&sc_desc, &pSC, &pDevice);
	if (FAILED(hr))
	{
		hr = D3D10CreateDeviceAndSwapChain(NULL, D3D10_DRIVER_TYPE_NULL, NULL, device_flag, D3D10_SDK_VERSION, &sc_desc, &pSC, &pDevice);
		if (FAILED(hr))
		{
			DebugBreak();
			return FALSE;
